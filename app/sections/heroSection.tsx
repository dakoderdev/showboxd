...
export default function Header({ randomShows }: HeroSectionProps) {
  ...
  return (
    <header className="relative w-full grid lg:grid-cols-2 h-fit bg-linear-to-b from-background to-black/90 sm:px-16">
      <div className="flex flex-col items-center sm:items-start self-center mb-16">
        ...
        <h1 ref={phraseRef} className="text-5xl min-[500px]:text-6xl text-center sm:text-start pt-3 pb-6 tracking-tighter font-semibold text-balance var(--font-geist-sans) transition-all duration-600">
          {HERO_PHRASES[0]}
        </h1>
      ...
      <div className="relative mb-12 h-fit">
        ...
        <Link href={`/shows/${randomShows[2]?.show_id ?? "#"}`} className="z-30 absolute right-1/3 sm:right-1/5 translate-x-1/3 pointer-events-none sm:pointer-events-auto sm:translate-x-1/4 top-12 w-64 h-96 bg-neutral-900 overflow-hidden shadow-md shadow-black/30 rounded-2xl before:inset-0 before:absolute before:pointer-events-none before:ring-2 before:ring-inset before:ring-white/10 before:rounded-2xl rotate-3 hover:translate-y-0 sm:hover:-translate-y-2 delay-75 transition-transform">
