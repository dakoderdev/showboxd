  ...
  return (
    <div className="flex gap-3 py-4 border-b border-white/5 last:border-0">
      <div className="shrink-0">
        <Image src={comment.user.profile_picture} alt={comment.user.username} width={36} height={36} className="rounded-full object-cover" />
      </div>
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-4 mt-2">
          ...
          <button className="text-white/40 hover:text-white/60 transition-colors text-xs">Reply</button>
        </div>
      </div>
    </div>
  );
...
export default async function ReviewPage({ params }: PageProps) {
  ...
  if (!review) {
    return <div className="min-h-dvh bg-neutral-950 text-white flex items-center justify-center">Review not found</div>;
  }
    ...
    <div className="min-h-dvh bg-neutral-950">
      <header className="sticky top-0 z-10 bg-neutral-950/80 backdrop-blur-md border-b border-white/5">
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center gap-3">
          <Link href={`/shows/${slug}`} className="flex items-center gap-2 text-white/60 hover:text-white transition-colors group">
      ...
      <main className="max-w-2xl mx-auto px-4 py-6">
        <div className="flex gap-4 pl-1 transition-colors mb-3 group">
          <div className="relative w-10 aspect-2/3 rounded-md overflow-hidden shrink-0">
            <Image src={show.img_vertical} alt={show.name} fill className="object-cover" />
          </div>
          <div className="flex flex-col justify-center">
            <h2 className="text-white font-semibold text-lg transition-colors">{show.name}</h2>
          ...
          <div className="p-5 border-b border-white/5">
            <div className="flex items-start justify-between gap-4">
              <div className="flex gap-3 items-center">
                <Image src={user.profile_picture} alt={user.username} width={44} height={44} className="rounded-full object-cover ring ring-white/10" />
                <div>
        ...
        <section className="mt-6">
          ...
          <div className="flex gap-3 mb-4 pb-4 border-b border-white/5">
            ...
            <div className="flex-1">
              <input type="text" placeholder="Add a comment..." className="w-full bg-neutral-900/50 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-blue-400/50 focus:ring-1 focus:ring-blue-400/20 transition-all" />
            </div>
          </div>
