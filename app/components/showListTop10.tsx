...
export default function ShowListTop10({ bestShows }: ShowListProps) {
  ...
  return (
    <section className="relative w-full px-0 pb-4 pt-12 mb-6 sm:px-12 md:px-17 border-t border-white/10 bg-background">
          ...
          {groups.map((group, groupIndex) => (
            <div key={groupIndex} className="grid grid-cols-2 gap-2">
              {group.map((item, index) => {
                ...
                return (
                  <Link key={item.show_id} href={`/shows/${item.show_id}`} className="group relative cursor-pointer w-full aspect-[2/3] shadow-md shadow-black/30 rounded-xl overflow-hidden before:inset-0 before:absolute before:pointer-events-none before:ring-2 before:ring-inset before:ring-white/10 before:bg-linear-30 before:from-black/60 before:to-transparent before:to-70% before:rounded-2xl">
